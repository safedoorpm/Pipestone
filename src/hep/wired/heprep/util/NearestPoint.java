...
/*
 * Obtained from http://wired.freehep.org/xref/hep/wired/heprep/util/NearestPoint.html
 * on 2010/10/24
 * Licensed under LGPL Version 2.1, February 1999
 * License available at http://wired.freehep.org/License.html
 */

import com.obtuse.ui.CubicBezierCurve;
 ...
 *
 * Calculations for the Bezier parts come from: "Solving the Nearest
 * Point-on-Curve Problem" and "A Bezier Curve-Based Root-Finder" by Philip J.
 * Schneider from "Graphics Gems", Academic Press, 1990.
 *
 * @author Mark Donszelmann
 * @version $Id: NearestPoint.java 8598 2006-08-14 20:38:02Z duns $
 */
@SuppressWarnings( {
        "UnusedDeclaration",
...
public class NearestPoint {
    ...
    public static double onLine( final Point2D p1, final Point2D p2, final Point2D pa, final Point2D pn) {
        ...
        if (dsq == 0) {
            ...
            double u = ((pa.getX() - p1.getX()) * dx + (pa.getY() - p1.getY())
                    * dy)
                    / dsq;
            if (u <= 0) {
                pn.setLocation(p1);
    ...
    /**
     * FindRoots : Given a 5th-degree equation in Bernstein-Bezier form, find
    ...
    private static int findRoots( final Point2D[] w, final int degree, final double[] t, final int depth) {
        ...
        Point2D[] left = new Point2D.Double[W_DEGREE + 1]; // New left and
        // right
    ...
    private static Point2D[] convertToBezierForm( final Point2D[] v, final Point2D pa) {
        ...
        for ( int i = 0; i < d.length; i += 1 ) {
            d[i] = new Point2D.Double();
        }
        double[][] cdTable = new double[3][4]; // Dot product of c, d
        Point2D[] w = new Point2D.Double[W_DEGREE + 1]; // Ctl pts of 5th-degree
        // curve
        for ( int i = 0; i < w.length; i += 1 ) {
            w[i] = new Point.Double();
        }
        ...
        for (int i = 0; i <= DEGREE - 1; i++) {
            d[i].setLocation(s * (v[i + 1].getX() - v[i].getX()), s
                    * (v[i + 1].getY() - v[i].getY()));
        }
        ...
        for (int row = 0; row <= DEGREE - 1; row++) {
            for (int column = 0; column <= DEGREE; column++) {
                cdTable[row][column] = (d[row].getX() * c[column].getX())
                        + (d[row].getY() * c[column].getY());
            }
        }
        ...
        for (int k = 0; k <= n + m; k++) {
            ...
            for (int i = lb; i <= ub; i++) {
                int j = k - i;
                w[i + j].setLocation(w[i + j].getX(), w[i + j].getY()
                        + cdTable[j][i] * cubicZ[j][i]);
            }
        }
    ...
    /**
     * CrossingCount : Count the number of times a Bezier control polygon
    ...
    private static boolean controlPolygonFlatEnough( final Point2D[] v, final int degree) {
        ...
        double c = v[0].getX() * v[degree].getY() - v[degree].getX()
                * v[0].getY();
    ...
    /*
     * ComputeXIntercept : Compute intersection of chord from first control
    ...
    private static Point2D bezier( final Point2D[] c, final int degree, final double t,
        ...
        for (int i = 1; i <= degree; i++) {
            for (int j = 0; j <= degree - i; j++) {
                p[i][j].setLocation((1.0 - t) * p[i - 1][j].getX() + t
                        * p[i - 1][j + 1].getX(), (1.0 - t)
                        * p[i - 1][j].getY() + t * p[i - 1][j + 1].getY());
            }
        }
    ...
    public static void main( final String[] args) {
        ...
        System.out.println("Point On Curve is (" + pn.getX() + ',' + pn.getY() + ") with dist "
                + Math.sqrt(distSq));
    }
}
