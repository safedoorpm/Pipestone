...
public class Circle {
	...
	public IntersectionInfo( @Nullable Point2D.Double[] points ) {
    ...
    public IntersectionInfo computeCircleCircleIntersection( Circle other ) {

	return computeCircleCircleIntersection( other, false );

    }

    public IntersectionInfo computeCircleCircleIntersection( Circle other, boolean testMode ) {
...
//	if ( d < Math.abs( r0 - r1 ) ) {
    ...
    @NotNull
    public IntersectionInfo intersects( @NotNull Circle other ) {
    ...
    public IntersectionInfo originCircleIntersects( @NotNull Circle other ) {
...
//	    Logger.logMsg( "circles internally touch:  f=" + f + ", intersect=" + intersect );
    ...
    private IntersectionInfo computeTwoIntersections( @NotNull Circle other ) {
...
//	Logger.logMsg( "cTT " + this + " vs " + other + ":  a2=" + a2 + ", cosTheta=" + cosTheta + ", sinTheta=" + sinTheta );
    ...
    private IntersectionInfo computeRotatedIntersections( @NotNull Circle other ) {
...
//	Logger.logMsg( "cRI " + this + " vs " + other + ":  xI=" + xIntersection + ", yI=" + yIntersection );
    ...
    @NotNull
    public MyPoint[] computeTangentLines( Point2D.Double p ) {
	...
	if ( info == null ) {
	    ...
	    throw new HowDidWeGetHereError( "circle D " + d + " is identical to circle C " + this + " when computing tangent lines from point P " + p + " to circle C (should be impossible)" );

	}

	if ( info.getCount() != 2 ) {

	    throw new HowDidWeGetHereError( "found " + info.getCount() + " intersection point" + ( info.getCount() == 1 ? "" : "s" ) + " between circle C " + this + " and circle D " + d + " when computing tangent lines from point P " + p + " to circle C (should be impossible)" );
    ...
    public Circle moveRelative( Point2D.Double delta ) {
    ...
    public Circle moveRelative( double xDelta, double yDelta ) {
    ...
    public Circle moveTo( Point2D.Double newLocation ) {
    ...
    public Circle moveTo( double xLocation, double yLocation ) {
    ...
    public Circle rotateDegrees( double degrees ) {
    ...
    public Circle rotateTheta( double theta ) {
    ...
    private static void doTangentScaling( Circle c, MyPoint p ) {
...
//	Logger.logMsg( "" + s_trialCount + ":  " + c + " vs " + p );
    ...
    private static void doTangentRotations( Circle c, MyPoint p ) {
    ...
    public static void doTangentXlates( Circle c, MyPoint p ) {
    ...
    private static void doOneTangent( Circle c, MyPoint p ) {
...
//	Logger.logMsg( "--- " + s_trialCount );
	...
	try {
		...
		Logger.logMsg( "*** got " + tangentPoints.length + " tangent point" + ( tangentPoints.length == 1 ? "" : "s" ) + " computing tangent lines from " + p + " to " + c );
		s_errorCount += 1;
    ...
    private static boolean checkTangentPoint( Circle c, MyPoint p, MyPoint tangentPoint ) {
...
//		Logger.logMsg( "ok" );
		...
		Logger.logMsg( "*** point " + tangentPoint + " is not a tangent point of point p " + p + " and circle c " + c + " (tangent line not perpendicular to radius line)" );
	    ...
	    Logger.logMsg( "*** point " + tangentPoint + " is not a tangent point of point p " + p + " and circle c " + c + " (tangent point not on circle)" );
...
//	}
    ...
    private static void doRotations( @NotNull Circle circle1, @NotNull Circle circle2, @Nullable MyPoint[] correctIntersections ) {
    ...
    private static void doMoves( @NotNull Circle circle1, @NotNull Circle circle2, @Nullable MyPoint[] correctIntersections ) {
    ...
    private static MyPoint[] moveIntersections( @Nullable MyPoint[] correctIntersections, double adjX, double adjY ) {
    ...
    private static MyPoint[] rotateIntersectionsDegrees( @Nullable MyPoint[] correctIntersections, double degrees ) {
    ...
    private static MyPoint[] rotateIntersectionsTheta( @Nullable MyPoint[] correctIntersections, double theta ) {
    ...
    private static void doOneSet( @NotNull Circle circle1, @NotNull Circle circle2, @Nullable MyPoint[] correctIntersections ) {
    ...
    private static void validate(
	    @NotNull Circle circle1,
	    @NotNull Circle circle2,
	    @NotNull IntersectionInfo info,
	    @Nullable MyPoint[] correctIntersections
    ) {
