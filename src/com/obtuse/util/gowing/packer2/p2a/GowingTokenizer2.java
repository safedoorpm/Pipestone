...
//	}
	...
	@NotNull
	public GowingPackable2ThingHolder2 createHolder( EntityName2 entityName, GowingToken2 valueToken, GowingUnPackerContext2 unPackerContext )
		throws GowingUnPacker2ParsingException {

	    GowingPackable2ThingHolder2 holder;
	    switch ( valueToken.type() ) {

		case NULL:
		    holder = new GowingNullHolder2( entityName );
		    break;

		case BOOLEAN:
		    holder = new GowingBooleanHolder2( entityName, valueToken.booleanValue(), true );
		    break;

		case BYTE:
		    holder = new GowingByteHolder2( entityName, valueToken.byteValue(), true );
		    break;

		case SHORT:
		    holder = new GowingShortHolder2( entityName, valueToken.shortValue(), true );
		    break;

		case INTEGER:
		    holder = new GowingIntegerHolder2( entityName, valueToken.intValue(), true );
		    break;

		case LONG:
		    holder = new GowingLongHolder2( entityName, valueToken.longValue(), true );
		    break;

		case FLOAT:
		    holder = new GowingFloatHolder2( entityName, valueToken.floatValue(), true );
		    break;

		case DOUBLE:
		    holder = new GowingDoubleHolder2( entityName, valueToken.doubleValue(), true );
		    break;

		case STRING:
		    holder = new GowingStringHolder2( entityName, valueToken.stringValue(), true );
		    break;
...
//		    }
		    ...
		    holder = new GowingEntityReferenceHolder2( entityName, valueToken.entityReference(), true );
		    break;
...
//	private final char[] _chars;
    ...
    public GowingTokenizer2( GowingUnPackerContext2 unPackerContext, LineNumberReader lineNumberReader ) {
	...
	try {
		...
		throw new GowingUnPacker2ParsingException( errorToken.stringValue(), errorToken );
...
//				Logger.logMsg( "ignoring '" + ( Character.isDefined( ch ) ? (char) ch : '?' ) + "'" );
    ...
    @SuppressWarnings("WeakerAccess")
    @NotNull
    public GowingToken2 finishCollectingEntityReference( boolean entityNameOk, GowingUnPackerContext2 unPackerContext, int typeId )
	    throws IOException, GowingUnPacker2ParsingException {
	...
	if ( ch == 'v' ) {
		...
		throw new GowingUnPacker2ParsingException( "version number is not positive (" + version + ")", entityVersionToken );
...
//	GowingToken2 entityNameString = null;
	...
	SortedSet<EntityName2> entityNames = new TreeSet<EntityName2>();
	...
	while ( true ) {
		...
		if ( entityNameOk ) {
			...
			throw new GowingUnPacker2ParsingException( "entity name string missing in entity name clause", entityNameClauseMarker );

		    }

		} else {

		    throw new GowingUnPacker2ParsingException( "entity name clause marker ('" + GowingConstants.ENTITY_NAME_CLAUSE_MARKER + "') not allowed in this context", entityNameClauseMarker );
...
//			    unPackerContext.findTypeByTypeReferenceId( typeId ),
	...
	try {
	    ...
	    GowingTokenizer2 tokenizer = new GowingTokenizer2( new StdGowingUnPackerContext2( new GowingTypeIndex2( "test GowingTokenizer2" ) ), new LineNumberReader( new FileReader( "test1.p2a" ) ) );
	    GowingToken2 token;
