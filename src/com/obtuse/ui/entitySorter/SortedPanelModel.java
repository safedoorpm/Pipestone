            ...
            for ( K key : _treeSorter.keySet() ) {
                ...
                for ( SortableEntityReference<K, E> entityReference : _treeSorter.getValues( key ) ) {
                    ...
                    if ( !entityReference.getActiveKey().equals( key ) ) {
                        ...
                        throw new IllegalArgumentException( "entity reference has key \"" +
                                                            entityReference.getActiveKey() +
                                                            "\" but should have key \"" +
                                                            key +
                                                            "\"" );
...
//		        Trace.event( "[" + ( index - 1 ) + "] = \"" + previousKey + "\", [" + index + "] = \"" + key + "\"" );
                        ...
                        if ( previousKey.compareTo( key ) > 0 ) {
                            ...
                            throw new IllegalArgumentException( "SortedPanelModel:  tree sorter is not sorted:  [" +
                                                                ( index - 1 ) +
                                                                "] = " +
                                                                previousKey +
                                                                ", [" +
    ...
    void adoptSortedPanel( final SortedPanel<K,E> owner ) {
        ...
        // It also avoids needing to consider the possibility that the caller is try to change our owner to who our owner already is.
        ...
        if ( owner == null || _owner == null ) {
            ...
            // We already have an owner which cannot be the owner that the caller provided to us
            // (thanks to the infinite recursion avoiding if test above).
    ...
    public int changeSortingKey( final K oldKey, final K newKey, final E myEntity ) { //} SortableEntityReference<K,E> myEntity ) {
...
//		    }
        ...
        if ( removedValueList.isEmpty() ) {
            ...
            throw new HowDidWeGetHereError( "entity being moved is not in our tree sorter:  oldKey=\"" +
                                            oldIndex +
                                            "\", newKey=\"" +
                                            newKey +
                                            "\", entity = {" +
    ...
    public String toString() {
        ...
        return "SortedPanelModel( \"" +
               getName() +
               "\", " +
               _treeSorter.size() +
               " elements" +
               ( hasOwner() ? ", owner=\"" + _owner.getName() + "\"" : ", unowned" ) +
               " )";
