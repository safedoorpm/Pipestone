...
//	Logger.logMsg( "L3C constructor for " + target.getName() );
...
//	    if ( tracingContainer ) {
//
//		Logger.pushNesting( "cache constructor" );
//
//		logIfWatched( "setting up a cache for container \"" + LinearLayoutUtil.fullName( target ) + "\" " /* + showAlignment( target.getAlignmentX(), target.getAlignmentY() ) */ );
//		LinearLayoutUtil.describeGuiEntity( "setting up cache", target );
//		Logger.pushNesting( "inner cache" );
//
//	    }
...
//		    if ( LinearLayoutUtil.isContainerOnWatchlist( target ) ) {
//
//			LinearLayoutUtil.describeGuiEntity( 0, c, false, false );
//
//		    }
//
//		}
...
//		}
		...
		if ( c instanceof LinearContainer3 ) {
		    ...
		    LinearContainer3 lc = (LinearContainer3)c;
...
//		    if ( tracingContainer || LinearLayoutUtil.isComponentOnWatchlist( c ) ) {
//
//			logIfWatched(
//				"post constraint sizes for " + LinearLayoutUtil.fullName( c ) + " are " +
//				ObtuseUtil.fDim( "min", min ) + ", " +
//				ObtuseUtil.fDim( "pref", pref ) + ", " +
//				ObtuseUtil.fDim( "max", max )
//			);
...
//		if ( isVertical() ) {
//
////		    if ( LinearLayoutUtil.isContainerOnWatchlist( target ) || LinearLayoutUtil.isComponentOnWatchlist( c ) ) {
////
////			logIfWatched( "vertical(\"" + c.getName() + "\"):  breadth=X, length=Y, xReq alignment is " + xReq.alignment + ", yReq alignment is " + yReq.alignment );
////
////		    }
//
//		} else {
...
////		    if ( LinearLayoutUtil.isContainerOnWatchlist( target ) || LinearLayoutUtil.isComponentOnWatchlist( c ) ) {
////
////			logIfWatched( "horizontal(\"" + c.getName() + "\"):  breadth=Y, length=X, yReq alignment is " + yReq.alignment + ", xReq alignment is " + xReq.alignment );
////
////		    }
//
//		}
...
//	    }
    ...
    private void workingOnWatchlistComponent( @SuppressWarnings({ "SameParameterValue", "unused" }) String what, @SuppressWarnings("unused") Component c ) {
...
//	    Logger.popNestingLevel( levelName );
    ...
    private void compressedTile(int allocated, long min, long pref, long max,
    ...
    private void expandedTile(int allocated, long min, long pref, long max,
	    SizeRequirements[] request,
	    int[] offsets, int[] spans,
	    boolean forward) {
